...
export const bigIntToUuid = (encodedId: bigint) => {
  ...
  if (isParsedNaN) throw new Error("bigint to UUID failed");

  const [a, b, c, d, e] = parsed.map((val, i) => {
    const length =
      i === 0 ? A_LENGTH_HEX : i === 4 ? E_LENGTH_HEX : BCD_LENGTH_HEX;
